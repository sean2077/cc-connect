...
.PHONY: build install run clean test test-fast test-full test-smoke test-e2e test-release test-release-local test-performance pre-test lint release release-all web
...
install: web
	go install $(_TAGS_FLAG) -ldflags "$(LDFLAGS)" $(CMD)
